...
class OsmStopsImporter(osmium.SimpleHandler):
	...
	def __init__(self, import_osm, import_haltestellen, stops_file, osm_file):
		...
		if self.import_haltestellen:
			NvbwStopsImporter(self.db).import_stops(stops_file)
		if self.import_osm:
...
def main(osmfile, stops_file):
	statshandler = OsmStopsImporter(import_osm = True, import_haltestellen = True, stops_file = stops_file, osm_file = osmfile)
