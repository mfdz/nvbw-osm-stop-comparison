	...
	def import_stops(self, stops_file):
		...
		with open(stops_file,'r',encoding='utf-8-sig') as csvfile:
			...
			cur.execute("""CREATE TABLE haltestellen_unified AS 
				...
				UNION
				SELECT s.District Landkreis, s.Municipality Gemeinde, LTRIM(SUBSTR(REPLACE(s.Name,',',' '), 1, instr(REPLACE(s.Name,',',' '),' '))) Ortsteil, LTRIM(SUBSTR(REPLACE(s.Name,',',' '), instr(REPLACE(s.Name,',',' '),' '))) Haltestelle, s.Name Haltestelle_lang, '' Haltebeschreibung, s.dhid GlobaleId, '' HalteTyp, '' gueltigAb, '' gueltigBis, s.Latitude lat, s.Longitude lon, 'Halt' Art, s.description Name_Steig, '' mode, NULL parent, '' match_state from zhv s WHERE s.type='S' AND s.dhid NOT IN (SELECT a.parent FROM zhv q JOIN zhv a ON q.parent=a.dhid WHERE type='Q') AND NOT s.state = 'Unserved' AND NOT s.condition='OutOfOrder'""")
